export default class MatteWidget {
  ...
  constructor(divElementId, config, answer = null, onAnswer, options) {
    // DEBUG if playback
    console.log(options && options.playback ? "Widget in playback mode" : "Widget in standard mode")
    ...
    //Speak on page load
    //***************************
    let loc = document.location.href.includes(":55") == false ? document.location.href : ""
    this.audioEl.src = loc + this.config.mp3BaseUrl + this.getFileNumstr() + ".m4a"
  ...
  reset_svg(ind, lg) {
    ...
    if (memb[0].classList.contains("colorized")) this.selected_class = "in_color"
    ...
    for (let src_el of all_src) {
      ...
      for (let lg_el = 0; lg_el < lg.length; lg_el++) {
        ...
        log_objid = this.is_numeric(log_objid[0]) ? (log_objid = "gr" + log_objid) : log_objid
        ...
        if (src_el.node.id == log_objid) {
          if (!this.already_replay && svg_obj.node.transform.animVal.length > 0) {
            ...
            if (log_objid in this.x_offset_diff == false) {
              ...
              this.x_offset_diff[log_objid] = this.x_offset[log_objid] - lg[lg_el].x[0]
            }

            //diff y
            if (log_objid in this.y_offset_diff == false) {
              this.y_offset[log_objid] = svg_obj.node.transform.animVal[0].matrix["f"]
              this.y_offset_diff[log_objid] = this.y_offset[log_objid] - lg[lg_el].y[0]
            }
          }
          ...
          if (
            ...
            if (log_objid in this.init_mx_a == false) {
              this.init_mx_a[log_objid] = src_el.node.transform.animVal[0].matrix["a"]
              this.init_mx_b[log_objid] = src_el.node.transform.animVal[0].matrix["b"]
              this.init_mx_c[log_objid] = src_el.node.transform.animVal[0].matrix["c"]
              this.init_mx_d[log_objid] = src_el.node.transform.animVal[0].matrix["d"]
            }
      ...
      case "move":
        if (typeof logg.x === "object") {
          logg.x.forEach((pos, index) => {
            ...
            setTimeout(
              () => {
                interval = index < logg.x.length ? logg.time[index + 1] - logg.time[index] : 0
                let pos_x =
                  index > 0 ? logg.x[index] + this.x_offset_diff[logg.obj] : this.x_offset[logg.obj]
    ...
    const actions = [
      {
        ...
        handler: () => {
          ...
          if (index == this.answer.length - 1 || (index == this.answer.length - 2 && skip))
            toggle = true
  ...
  parseSVG(svgResp) {
    ...
    doc.querySelector("svg").setAttribute("width", this.width_svg > 0 ? this.width_svg : "100%")
    doc.querySelector("svg").setAttribute("height", this.height_svg > 0 ? this.height_svg : "100%")
    window.svgdoc = doc
    this.svgimage = this.svgelement.svg(new XMLSerializer().serializeToString(doc))
  }
  ...
  runscript() {
    ...
    this.size_src_obj =
      ...
      SVG().select(".source").members[0].node.transform.animVal.length > 0
        ? SVG().select(".source").members[0].node.transform.animVal[0].matrix["a"]
        : 0
    ...
    SVG.select(".next").on("click", (event) => {
      ...
      if (event.currentTarget.classList.contains("speak")) {
        this.audioEl.src = this.config.mp3BaseUrl + this.getFileNumstr() + "next.m4a"
    ...
    SVG.select(".speak").on("click", (event) => {
      ...
      if (!event.currentTarget.classList.contains("next")) {
        ...
        let filenum = event.currentTarget.classList.contains("select") ? "" : this.getFileNumstr()
      ...
      onDragEnd: function (e) {
        ...
        while (--i > -1) {
          if (this.hitTest(widgetThis.targets.members[i].node)) {
            ...
            //logger hendelser
            widgetThis.setEventdata("hit", this, widgetThis.targets.members[i].node.id, widgetThis)

            var pos = widgetThis.targets.members[i].bbox()
            var t = pos.x2 * 0.97 + " " + pos.cy * 0.95

            //ser om klassen disappear finnes i src svg-objekt
            var disappear = widgetThis.targets.members[i].node.classList.contains("eat")
              ? true
              : false
            if (disappear) {
            ...
            //Speak on target hit
            //***************************
            if (widgetThis.targets.members[i].node.classList.contains("speak_when_hit")) {
              widgetThis.audioEl.src =
                widgetThis.config.mp3BaseUrl + widgetThis.getFileNumstr() + "hit.m4a"
  ...
  setEventdata(evtype, ev, hitobj) {
    let trgobj = ev.currentTarget != null ? ev.currentTarget : ev.target != null ? ev.target : ""
    ...
    const eventen = {
      ...
      val:
        trgobj.attributes["selectvalue"] == null ? "emp" : trgobj.attributes["selectvalue"].value,
      ...
      tdiff:
        ...
        this.answer[this.answer.length - 1].time != null
          ? (Date.now() - an[an.length - 1].time[an[an.length - 1].time.length - 1]) / 1000
          : "",
      hit: hitobj,
    }
